...
def run_on_images(args, img_paths):
    ...
    for i, img_path in enumerate(tqdm(img_paths, desc="Processing images")):
        ...
        person_confs = person_detection_result.boxes.conf.cpu().numpy()
        if len(person_bboxes):
            _wh = person_bboxes[:, 2:4] - person_bboxes[:, 0:2]
            _ok = (_wh[:, 0] > 1) & (_wh[:, 1] > 1)
            person_bboxes = person_bboxes[_ok]; person_confs = person_confs[_ok]
        ...
        if len(person_bboxes) == 0:
            ...
            try:
                ...
                print('POSE FAIL', img_path, _e, flush=True)
                save_result_metadata_pickle(None, person_detection_dict, out_pkl_path)
                continue
            save_result_metadata_pickle(batch_results, person_detection_dict, out_pkl_path)

        # === Visualize every N samples ===
        if False:  # viz disabled: crashes on some heatmaps + writes face jpgs into output
            if len(person_bboxes) == 0:
...
def main(args):
    ...
    ray.init(_temp_dir="/data2/mcfrank/ray_tmp")
