...
debug = False
import sys as _sys
out_vis_dir = _sys.argv[1] if len(_sys.argv) > 1 else '/ccn2/dataset/babyview/2025.2/outputs/pose_1fps'
output_csv_path = _sys.argv[2] if len(_sys.argv) > 2 else '/ccn2/dataset/babyview/2025.2/outputs/pose_1fps_bbox_limbs.csv'
num_processes = 16
...
def extract_list_of_metadata_dicts_from_pkl(pkl_path):
    ...
    with open(pkl_path, 'rb') as f:
        ...
        for i, per_person_pose_dict in pose_dict.items():
            ...
            person_bbox_conf = round(float(person_bbox_conf_list[i]), 2)
...
def create_dataframe(vis_pkl_paths):
    ...
    _rows = []
    for pkl_path in tqdm(vis_pkl_paths, desc="Processing PKL files"):
        _rows.extend(extract_list_of_metadata_dicts_from_pkl(pkl_path))
